...
#include "stim/simulators/frame_simulator_GPU.h"
...
inline void for_each_target_pair(GPUFrameSimulator<W> &sim, const CircuitInstruction &target_data, FUNC body) {
...
template <size_t W>
GPUFrameSimulator<W>::GPUFrameSimulator(
    CircuitStats circuit_stats, GPUFrameSimulatorMode mode, size_t batch_size, std::mt19937_64 &&rng)
    : num_qubits(0),
      num_observables(0),
...
template <size_t W>
void GPUFrameSimulator<W>::configure_for(
    CircuitStats new_circuit_stats, GPUFrameSimulatorMode new_mode, size_t new_batch_size) {
    bool storing_all_measurements = new_mode == GPUFrameSimulatorMode::STORE_MEASUREMENTS_TO_MEMORY ||
                                    new_mode == GPUFrameSimulatorMode::STORE_EVERYTHING_TO_MEMORY;
    bool storing_all_detections = new_mode == GPUFrameSimulatorMode::STORE_DETECTIONS_TO_MEMORY ||
                                  new_mode == GPUFrameSimulatorMode::STORE_EVERYTHING_TO_MEMORY;
    bool storing_any_detections = new_mode == GPUFrameSimulatorMode::STORE_DETECTIONS_TO_MEMORY ||
                                  new_mode == GPUFrameSimulatorMode::STORE_EVERYTHING_TO_MEMORY ||
                                  new_mode == GPUFrameSimulatorMode::STREAM_DETECTIONS_TO_DISK;
...
template <size_t W>
void GPUFrameSimulator<W>::ensure_safe_to_do_circuit_with_stats(const CircuitStats &stats) {
...
template <size_t W>
void GPUFrameSimulator<W>::safe_do_circuit(const Circuit &circuit, uint64_t repetitions) {
...
template <size_t W>
void GPUFrameSimulator<W>::safe_do_instruction(const CircuitInstruction &instruction) {
    ensure_safe_to_do_circuit_with_stats(instruction.compute_stats(nullptr));
    do_gate(instruction);
}

template <size_t W>
void GPUFrameSimulator<W>::xor_control_bit_into(uint32_t control, simd_bits_range_ref<W> target) {
...
template <size_t W>
void GPUFrameSimulator<W>::reset_all() {
    x_table.clear();
    if (guarantee_anticommutation_via_frame_randomization) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_circuit(const Circuit &circuit) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_MX(const CircuitInstruction &inst) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_MY(const CircuitInstruction &inst) {
    m_record.reserve_noisy_space_for_results(inst, rng);
    for (auto t : inst.targets) {
        auto q = t.qubit_value();  // Flipping is ignored because it is accounted for in the reference sample.
        x_table[q] ^= z_table[q];
...
template <size_t W>
void GPUFrameSimulator<W>::do_MZ(const CircuitInstruction &inst) {
    m_record.reserve_noisy_space_for_results(inst, rng);
    for (auto t : inst.targets) {
        auto q = t.qubit_value();  // Flipping is ignored because it is accounted for in the reference sample.
        m_record.xor_record_reserved_result(x_table[q]);
...
template <size_t W>
void GPUFrameSimulator<W>::do_RX(const CircuitInstruction &inst) {
    for (auto t : inst.targets) {
        auto q = t.data;
        if (guarantee_anticommutation_via_frame_randomization) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_DETECTOR(const CircuitInstruction &inst) {
    if (keeping_detection_data) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_OBSERVABLE_INCLUDE(const CircuitInstruction &inst) {
    if (keeping_detection_data) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_RY(const CircuitInstruction &inst) {
    for (auto t : inst.targets) {
        auto q = t.data;
        if (guarantee_anticommutation_via_frame_randomization) {
            z_table[q].randomize(z_table[q].num_bits_padded(), rng);
...
template <size_t W>
void GPUFrameSimulator<W>::do_RZ(const CircuitInstruction &inst) {
    for (auto t : inst.targets) {
        auto q = t.data;
        x_table[q].clear();
        if (guarantee_anticommutation_via_frame_randomization) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_MRX(const CircuitInstruction &target_data) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_MRY(const CircuitInstruction &target_data) {
    // Note: Caution when implementing this. Can't group the resets. because the same qubit target may appear twice.
    m_record.reserve_noisy_space_for_results(target_data, rng);
    for (auto t : target_data.targets) {
        auto q = t.qubit_value();  // Flipping is ignored because it is accounted for in the reference sample.
...
template <size_t W>
void GPUFrameSimulator<W>::do_MRZ(const CircuitInstruction &target_data) {
    // Note: Caution when implementing this. Can't group the resets. because the same qubit target may appear twice.
    m_record.reserve_noisy_space_for_results(target_data, rng);
    for (auto t : target_data.targets) {
        auto q = t.qubit_value();  // Flipping is ignored because it is accounted for in the reference sample.
...
template <size_t W>
void GPUFrameSimulator<W>::do_I(const CircuitInstruction &target_data) {
}

template <size_t W>
PauliString<W> GPUFrameSimulator<W>::get_frame(size_t sample_index) const {
...
template <size_t W>
void GPUFrameSimulator<W>::set_frame(size_t sample_index, const PauliStringRef<W> &new_frame) {
    assert(sample_index < batch_size);
...
template <size_t W>
void GPUFrameSimulator<W>::do_H_XZ(const CircuitInstruction &target_data) {
    for (auto t : target_data.targets) {
        auto q = t.data;
...
template <size_t W>
void GPUFrameSimulator<W>::do_H_XY(const CircuitInstruction &target_data) {
    for (auto t : target_data.targets) {
        auto q = t.data;
        z_table[q] ^= x_table[q];
    }
...
template <size_t W>
void GPUFrameSimulator<W>::do_H_YZ(const CircuitInstruction &target_data) {
    for (auto t : target_data.targets) {
        auto q = t.data;
        x_table[q] ^= z_table[q];
    }
...
template <size_t W>
void GPUFrameSimulator<W>::do_C_XYZ(const CircuitInstruction &target_data) {
    for (auto t : target_data.targets) {
        auto q = t.data;
        x_table[q] ^= z_table[q];
        z_table[q] ^= x_table[q];
...
template <size_t W>
void GPUFrameSimulator<W>::do_C_ZYX(const CircuitInstruction &target_data) {
    for (auto t : target_data.targets) {
        auto q = t.data;
        z_table[q] ^= x_table[q];
        x_table[q] ^= z_table[q];
...
template <size_t W>
void GPUFrameSimulator<W>::single_cx(uint32_t c, uint32_t t) {
...
template <size_t W>
void GPUFrameSimulator<W>::single_cy(uint32_t c, uint32_t t) {
    c &= ~TARGET_INVERTED_BIT;
    t &= ~TARGET_INVERTED_BIT;
    if (!((c | t) & (TARGET_RECORD_BIT | TARGET_SWEEP_BIT))) {
        x_table[c].for_each_word(
...
template <size_t W>
void GPUFrameSimulator<W>::do_ZCX(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
    assert((targets.size() & 1) == 0);
    for (size_t k = 0; k < targets.size(); k += 2) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_ZCY(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
    assert((targets.size() & 1) == 0);
    for (size_t k = 0; k < targets.size(); k += 2) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_ZCZ(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
    assert((targets.size() & 1) == 0);
    for (size_t k = 0; k < targets.size(); k += 2) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_SWAP(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
    assert((targets.size() & 1) == 0);
    for (size_t k = 0; k < targets.size(); k += 2) {
        size_t q1 = targets[k].data;
...
template <size_t W>
void GPUFrameSimulator<W>::do_ISWAP(const CircuitInstruction &target_data) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_CXSWAP(const CircuitInstruction &target_data) {
    for_each_target_pair(
        *this, target_data, [](simd_word<W> &x1, simd_word<W> &z1, simd_word<W> &x2, simd_word<W> &z2) {
            z2 ^= z1;
            z1 ^= z2;
...
template <size_t W>
void GPUFrameSimulator<W>::do_CZSWAP(const CircuitInstruction &target_data) {
    for_each_target_pair(
        *this, target_data, [](simd_word<W> &x1, simd_word<W> &z1, simd_word<W> &x2, simd_word<W> &z2) {
            std::swap(z1, z2);
            std::swap(x1, x2);
...
template <size_t W>
void GPUFrameSimulator<W>::do_SWAPCX(const CircuitInstruction &target_data) {
    for_each_target_pair(
        *this, target_data, [](simd_word<W> &x1, simd_word<W> &z1, simd_word<W> &x2, simd_word<W> &z2) {
            z1 ^= z2;
            z2 ^= z1;
...
template <size_t W>
void GPUFrameSimulator<W>::do_SQRT_XX(const CircuitInstruction &target_data) {
    for_each_target_pair(
        *this, target_data, [](simd_word<W> &x1, simd_word<W> &z1, simd_word<W> &x2, simd_word<W> &z2) {
            auto dz = z1 ^ z2;
            x1 ^= dz;
...
template <size_t W>
void GPUFrameSimulator<W>::do_SQRT_YY(const CircuitInstruction &target_data) {
    for_each_target_pair(
        *this, target_data, [](simd_word<W> &x1, simd_word<W> &z1, simd_word<W> &x2, simd_word<W> &z2) {
            auto d = x1 ^ z1 ^ x2 ^ z2;
            x1 ^= d;
...
template <size_t W>
void GPUFrameSimulator<W>::do_SQRT_ZZ(const CircuitInstruction &target_data) {
    for_each_target_pair(
        *this, target_data, [](simd_word<W> &x1, simd_word<W> &z1, simd_word<W> &x2, simd_word<W> &z2) {
            auto dx = x1 ^ x2;
            z1 ^= dx;
...
template <size_t W>
void GPUFrameSimulator<W>::do_XCX(const CircuitInstruction &target_data) {
    for_each_target_pair(
        *this, target_data, [](simd_word<W> &x1, simd_word<W> &z1, simd_word<W> &x2, simd_word<W> &z2) {
            x1 ^= z2;
            x2 ^= z1;
...
template <size_t W>
void GPUFrameSimulator<W>::do_XCY(const CircuitInstruction &target_data) {
    for_each_target_pair(
        *this, target_data, [](simd_word<W> &x1, simd_word<W> &z1, simd_word<W> &x2, simd_word<W> &z2) {
            x1 ^= x2 ^ z2;
            x2 ^= z1;
...
template <size_t W>
void GPUFrameSimulator<W>::do_XCZ(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
    assert((targets.size() & 1) == 0);
    for (size_t k = 0; k < targets.size(); k += 2) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_YCX(const CircuitInstruction &target_data) {
    for_each_target_pair(
        *this, target_data, [](simd_word<W> &x1, simd_word<W> &z1, simd_word<W> &x2, simd_word<W> &z2) {
            x2 ^= x1 ^ z1;
            x1 ^= z2;
...
template <size_t W>
void GPUFrameSimulator<W>::do_YCY(const CircuitInstruction &target_data) {
    for_each_target_pair(
        *this, target_data, [](simd_word<W> &x1, simd_word<W> &z1, simd_word<W> &x2, simd_word<W> &z2) {
            auto y1 = x1 ^ z1;
            auto y2 = x2 ^ z2;
...
template <size_t W>
void GPUFrameSimulator<W>::do_YCZ(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
    assert((targets.size() & 1) == 0);
    for (size_t k = 0; k < targets.size(); k += 2) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_DEPOLARIZE1(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
...
template <size_t W>
void GPUFrameSimulator<W>::do_DEPOLARIZE2(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
...
template <size_t W>
void GPUFrameSimulator<W>::do_X_ERROR(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
    RareErrorIterator::for_samples(target_data.args[0], targets.size() * batch_size, rng, [&](size_t s) {
        auto target_index = s / batch_size;
        auto sample_index = s % batch_size;
...
template <size_t W>
void GPUFrameSimulator<W>::do_Y_ERROR(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
    RareErrorIterator::for_samples(target_data.args[0], targets.size() * batch_size, rng, [&](size_t s) {
        auto target_index = s / batch_size;
        auto sample_index = s % batch_size;
...
template <size_t W>
void GPUFrameSimulator<W>::do_Z_ERROR(const CircuitInstruction &target_data) {
    const auto &targets = target_data.targets;
    RareErrorIterator::for_samples(target_data.args[0], targets.size() * batch_size, rng, [&](size_t s) {
        auto target_index = s / batch_size;
        auto sample_index = s % batch_size;
...
template <size_t W>
void GPUFrameSimulator<W>::do_MPP(const CircuitInstruction &target_data) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_SPP(const CircuitInstruction &target_data) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_SPP_DAG(const CircuitInstruction &target_data) {
    decompose_spp_or_spp_dag_operation(target_data, num_qubits, false, [&](const CircuitInstruction &inst) {
        safe_do_instruction(inst);
    });
}
...
template <size_t W>
void GPUFrameSimulator<W>::do_PAULI_CHANNEL_1(const CircuitInstruction &target_data) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_PAULI_CHANNEL_2(const CircuitInstruction &target_data) {
    tmp_storage = last_correlated_error_occurred;
...
template <size_t W>
void GPUFrameSimulator<W>::do_CORRELATED_ERROR(const CircuitInstruction &target_data) {
    last_correlated_error_occurred.clear();
    do_ELSE_CORRELATED_ERROR(target_data);
}

template <size_t W>
void GPUFrameSimulator<W>::do_ELSE_CORRELATED_ERROR(const CircuitInstruction &target_data) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_HERALDED_PAULI_CHANNEL_1(const CircuitInstruction &inst) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_HERALDED_ERASE(const CircuitInstruction &inst) {
    auto nt = inst.targets.size();
    m_record.reserve_space_for_results(nt);
    for (size_t k = 0; k < nt; k++) {
        m_record.storage[m_record.stored + k].clear();
...
template <size_t W>
void GPUFrameSimulator<W>::do_MXX_disjoint_controls_segment(const CircuitInstruction &inst) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_MYY_disjoint_controls_segment(const CircuitInstruction &inst) {
    // Transform from 2 qubit measurements to single qubit measurements.
...
template <size_t W>
void GPUFrameSimulator<W>::do_MZZ_disjoint_controls_segment(const CircuitInstruction &inst) {
    // Transform from 2 qubit measurements to single qubit measurements.
...
template <size_t W>
void GPUFrameSimulator<W>::do_MXX(const CircuitInstruction &inst) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_MYY(const CircuitInstruction &inst) {
    decompose_pair_instruction_into_disjoint_segments(inst, num_qubits, [&](CircuitInstruction segment) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_MZZ(const CircuitInstruction &inst) {
    decompose_pair_instruction_into_disjoint_segments(inst, num_qubits, [&](CircuitInstruction segment) {
...
template <size_t W>
void GPUFrameSimulator<W>::do_MPAD(const CircuitInstruction &inst) {
    m_record.reserve_noisy_space_for_results(inst, rng);
...
template <size_t W>
void GPUFrameSimulator<W>::do_gate(const CircuitInstruction &inst) {
    switch (inst.gate_type) {
        ...
        default:
            throw std::invalid_argument("Not implemented in GPUFrameSimulator<W>::do_gate: " + inst.str());
    }
}
