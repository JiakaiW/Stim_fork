...
#ifndef _STIM_SIMULATORS_FRAME_SIMULATOR_GPU_H
#define _STIM_SIMULATORS_FRAME_SIMULATOR_GPU_H
...
enum class GPUFrameSimulatorMode {
...
struct GPUFrameSimulator {
    ...
    /// Constructs a GPUFrameSimulator capable of simulating a circuit with the given size stats.
    ///
    /// Args:
    ...
    GPUFrameSimulator(CircuitStats circuit_stats, GPUFrameSimulatorMode mode, size_t batch_size, std::mt19937_64 &&rng);
    GPUFrameSimulator() = delete;

    PauliString<W> get_frame(size_t sample_index) const;
    void set_frame(size_t sample_index, const PauliStringRef<W> &new_frame);
    void configure_for(CircuitStats new_circuit_stats, GPUFrameSimulatorMode new_mode, size_t new_batch_size);
...
#include "stim/simulators/frame_simulator_GPU.inl"
